...
class HDFSUploadTabV4Clean:
    ...
    def create_ui(self):
        ...
        self.log_text.tag_config('warning', foreground='#FFA657')
        self.log_text.tag_config('info', foreground='#79C0FF')
        
        self.log("📤 HDFS Upload Manager ready\n", 'info')
    ...
    def remove_file(self, filepath):
        ...
        if filepath in self.selected_files:
            ...
            self.update_file_list()
            self.log(f"Removed: {Path(filepath).name}\n", 'warning')
    ...
    def test_connection(self):
        ...
        self.log("Testing HDFS connection...\n", 'info')
        ...
        def test():
            try:
                ...
                if result.returncode == 0:
                    ...
                    self.status_badge.config(text="● Connected", bg='#DDF4E6', fg='#1A7F37')
                else:
                    self.log(f"✗ Connection failed: {result.stderr}\n", 'error')
                    self.status_badge.config(text="● Error", bg='#FFE4E6', fg='#CF222E')
    ...
    def start_upload(self):
        ...
        self.log(f"\n▶ Starting upload of {len(self.selected_files)} file(s)...\n", 'info')
        ...
        def upload():
            ...
            for filepath in self.selected_files:
                try:
                    ...
                    subprocess.run(copy_cmd, check=True, capture_output=True, timeout=60)
                    
                    # Put to HDFS
                    hdfs_cmd = ['docker', 'exec', container, 'hdfs', 'dfs', '-put', 
    ...
    def stop_upload(self):
        ...
        self.status_badge.config(text="● Stopped", bg='#FFF8C5', fg='#9A6700')
