...
class ResilientDataLoader:
    ...
    def load_or_resume(spark, input_path, checkpoint_path, progress_tracker):
        ...
        if (checkpoint_exists and checkpoint_non_empty) or progress_tracker.is_completed("load_data"):
            ...
            metadata = progress_tracker.progress.get("load_data", {}).get("metadata", {})
        ...
        print(f"Format: JSON (Spotify MPD)")
        
        # Load JSON directly - MPD format (PROVEN WORKING METHOD)
        print(f"\n🔍 Loading JSON data...")
        start_time = time.time()
        
        try:
            # Fix: Handle path correctly - don't add /*.json if already present
            ...
            # Use the EXACT method from working code
        ...
        # Step 2: Explode tracks and create interactions
        ...
        interactions_df = (
            ...
            .dropDuplicates(["playlist_id", "track_uri"])  # Remove duplicates
            .repartition(36, "playlist_id")  # Repartition for balance
        )
        
        print(f"   ✅ Converted to playlist_id/track_uri format")
        ...
        progress_tracker.progress["load_data"] = {
            "completed": True,
            "metadata": {
...
class ResilientFilter:
    ...
    def filter_or_resume(spark, raw_df, checkpoint_path, progress_tracker, min_user=5, min_item=10):
        ...
        # Use playlist_id (from working code) instead of pid
        user_counts = raw_df.groupBy("playlist_id").count().withColumnRenamed("count", "user_count")
        ...
        filtered = raw_df \
            ...
            .repartition(36, "playlist_id")  # Add repartition like working code
...
class ResilientIndexer:
    ...
    def index_or_resume(spark, filtered_df, checkpoint_path, progress_tracker, save_indexers=True, indexer_base_path=None):
        ...
        # Repartition before indexing (from working code)
        print(f"\n🔄 Repartitioning data...")
        filtered_df = filtered_df.repartition(36, "playlist_id")
        
        print(f"\n🔢 Indexing users (playlists)...")
        # Use playlist_id as input column (keep consistency)
        user_indexer = StringIndexer(
            inputCol="playlist_id", 
            outputCol="playlist_idx", 
            handleInvalid="skip",
            stringOrderType="frequencyDesc"  # Add ordering from working code
        )
        ...
        indexed = indexed.select(
            col("playlist_id"),  # Keep original ID
